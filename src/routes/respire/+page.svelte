			...
			// Handle sound based on playback state
				if (!isPlaying) {
					...
					if (project.getAudioContext().state !== 'suspended') {
						project.getAudioContext().resume().then(() => {
		...
		project = new p5(
			(p: p5) => {
				...
				p.draw = () => {
					...
					p.textSize(16);
					p.text(`${minutes}:${seconds.toString().padStart(2, '0')} remaining`, width / 2, height - 40);
