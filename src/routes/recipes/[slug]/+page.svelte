...
{#if data.recipe}
	...
	<div class="flex flex-col gap-6 p-4">
		<h2 class="flex w-fit items-center gap-2 text-center text-2xl">
			<ChefHat />{data.recipe.name}
		</h2>
		<div class="flex flex-col gap-2 sm:gap-4 md:flex-row">
			<div class="flex items-center gap-2">
				<Flame /> Cook time: {data.recipe.cookTime} min
			</div>
			<div class="flex items-center gap-2">
		...
		{#if data.recipe.notes}
			<div class="border border-solid border-emerald-200 p-2">
				...
				<p class="text-sm whitespace-pre-line">{@html data.recipe.notes}</p>
			</div>
		{/if}
