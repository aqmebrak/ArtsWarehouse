...
export class EnemyManager {
    ...
    spawnSlimeGroup() {
        ...
        for (let i = 0; i < this.slimesPerSpawn; i++) {
            ...
            const x = Phaser.Math.Between(area.x, area.x + area.width) + Phaser.Math.Between(-10, 10);
            const y = Phaser.Math.Between(area.y, area.y + area.height) + Phaser.Math.Between(-10, 10);
    ...
    increaseDifficulty() {
        ...
        console.log(`Difficulty increased: Spawning ${this.slimesPerSpawn} slimes every ${this.spawnDelay / 1000}s`);
    }
