...
export class Slime {
	...
	update() {
		...
		if (closestDoor) {
			...
			if (distanceToDoor <= this.attackRange + 5) {
				// Target door if close
	...
	private moveTowardTarget(directionX: number, directionY: number, length: number) {
		if (length > 0) {
			...
			this.slime.setVelocity(normalizedX * this.speed, normalizedY * this.speed);
	...
	takeDamage(amount: number) {
		...
		this.scene.time.delayedCall(100, () => {
			if (this.slime.active) {
				// Check if still active before clearing tint
				this.slime.clearTint();
			}
		});
