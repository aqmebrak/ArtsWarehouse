...
export class Basecamp {
	...
	createWalls() {
		...
		if (this.walls) {
			...
			wall = this.walls.create(
				left + bottomWallLeftWidth / 2,
				bottom - this.wallThickness / 2,
				'wall'
			);
			wall.setSize(bottomWallLeftWidth, this.wallThickness).setOrigin(0.5, 0.5).refreshBody();

			wall = this.walls.create(
				bottomWallRightStart + bottomWallRightWidth / 2,
				bottom - this.wallThickness / 2,
				'wall'
			);
			wall.setSize(bottomWallRightWidth, this.wallThickness)
				.setOrigin(0.5, 0.5)
				.refreshBody();
