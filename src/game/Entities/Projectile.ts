...
export class Projectile {
    ...
    constructor(scene: Game, x: number, y: number, targetX: number, targetY: number, damage: number) {
        this.scene = scene;
        this.damage = damage;
        ...
        if (this.scene.projectiles && this.scene.projectiles instanceof Phaser.Physics.Arcade.Group) {
    ...
    setupPhysics(targetX: number, targetY: number) {
        ...
        const angle = Phaser.Math.Angle.Between(this.projectileSprite.x, this.projectileSprite.y, targetX, targetY);
        ...
        this.scene.physics.velocityFromRotation(angle, this.speed, this.projectileSprite.body?.velocity);
